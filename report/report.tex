...
I utilised several techniques to enforce single-instruction-multiple-data (SIMD) vectorization.
Firstly, I compiled \texttt{d2q9-bgk.c} with the \texttt{xAVX2} option to direct the compiler to optimise for Intel processors that support Advanced Vector Extensions 2 (AVX2), which BC4's compute nodes do \cite{lenovo}.

Within my code, I implemented the \texttt{\#pragma omp simd} pragma to vectorise the inner loop within each timestep.
This pragma indicates to the compiler that SIMD instructions can be used to execute iterations of the loop concurrently.
