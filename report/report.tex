...
Instead, all new values of cells were written to the \texttt{cells\_new} array, and the pointers of \texttt{cells} and \texttt{cells\_new} were swapped each timestep.
...
To eliminate a several unnecessary division operations, I precalculated several values: \texttt{c\_sq\_r}, the reciprocal of $c^2$; \texttt{two\_c\_sq\_r}, the reciprocal of $2c^2$; and \texttt{two\_c\_sq\_sq\_r}, the reciprocal of $2c^4$, where $c$ was the speed of sound.
...
This was because the AoS format had better memory locality in the program's mostly unvectorized form.
...
I implemented the \texttt{\#pragma omp simd} pragma to direct to the compiler to utilise SIMD instructions to execute operations within the inner loop in \texttt{timestep}.
...
I did not have to compile with the \texttt{-qopenmp-simd} option explicitly, since it was enabled by default at the \texttt{-Ofast} optimisation level \cite{icc}.
...
There is a significant speedup over the vectorized implementation because the program was able to utilise a larger amount of L1 and L2 cache, and execute iterations of the inner loop in parallel.
...
The NUMA-aware implementation produced a reasonable performance boost, by virtue of reducing the average time to access a memory location.
...
Secondly, inspecting the Intel Advisor memory metrics highlighted that the parallel program did not have to interact with the DRAM, which had a lower bandwidth, as often.
...
This was important since in real-world scenarios people frequently sacrifice small amounts of performance for a simpler implementation.
...
Methods exist to partially rectify these problems, however they are directly contradictory to the desired simplicity of the experiment.
In conclusion, one cannot achieve a comparable level of performance from a simpler implementation using Go's core features.
