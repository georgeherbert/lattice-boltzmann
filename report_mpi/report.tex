...
In my OpenMP implementation, the overhead arose from the creation and synchronisation of threads each timestep.
In my MPI implementation, the overhead arose from the initialisation of the MPI environment and the halo exchange.
For smaller grids, the MPI initialisation dominated the execution time of the program; in fact, for the $128 \times 128$ grid, the initialisation section of the program constituted 70\% of the total execution time.
Moreover, the initialisation time actually initially increased as the number of processes increased.
This meant that the MPI implementation scaled significantly worse for the smaller grid.
However, the compute time of my MPI implementation was actually shorter for each grid size, due to the overhead introduced by OpenMP.
Therefore, for the larger grid size, in which the compute time constituted a larger proportion of the total runtime, both implementations scaled equally as well.

\subsection{Implementation}

Having understood of how my OpenMP and MPI implementations scaled, I hypothesised that a hybrid implementation would reduce the execution times for small grids due to the proportionally large initialisation time of the MPI environment.
Furthermore, I also hypothesised that a hybrid implementation would reduce the execution times for grids with a small $y$-length, but a significantly large $x$-length because I believed the overhead introduced by the high number of large halo exchanges would be more significant than the overhead introduced by creating and synchronising OpenMP threads; I produced an additional grid to test this hypothesis.
...
The OpenCL implementation was slower for the smaller two rid sizes but faster for the larger two grid size.
